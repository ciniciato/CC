...
ImageData.prototype.getGrayScaleChannel = function(scale){
	var newImg = new Uint8ClampedArray(this.width*this.height), i = 0, iN = 0, len = this.data.length, factor=256/scale;
	for (; i < len; i+=4, iN++)
		newImg[iN] = factor*Math.round((0.298 * this.data[i] + 0.586 * this.data[i+1] + 0.114 * this.data[i+2])/factor);
	return newImg;
}
...
ImageData.prototype.getGrayChannelNormalized = function(){
	...
	for (; i < len; i+=4, iN++)
		newImg[iN] = (0.298 * this.data[i] + 0.586 * this.data[i+1] + 0.114 * this.data[i+2])/10;
	return newImg;
}
